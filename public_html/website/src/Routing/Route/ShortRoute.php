...
class ShortRoute extends \Cake\Routing\Route\Route
{
    
    var $routes = [];
    var $s = ['ä','ü','ö','Ä','Ü','Ö','ß'];
    var $r = ['ae','ue','oe','Ae','Ue','Oe','ss'];
    var $ext = '.html';
    ...
    public function parse($url, $method = '')
    {
        ...
        if(array_key_exists('match', $params) && is_string($params['match']) && $params['match'] == 'pages'){
            if(count($params['pass']) == 0){ // f.e. www.test.at
                ...
                if(count($params['pass']) == 2 && !array_key_exists($params['pass'][0], Configure::read('translations')) && strlen($params['pass'][1]) == 2 && ctype_alpha($params['pass'][1]) && array_key_exists($params['pass'][1], Configure::read('translations'))){ // f.e. www.test.at/structure/en
                    $params['structure'] = $params['pass'][0];
                    $params['language'] = $params['pass'][1];
                    $params['match'] = 'structure-language';
                }else if(count($params['pass']) > 2 && !array_key_exists($params['pass'][0], Configure::read('translations')) && strlen($params['pass'][1]) == 2 && ctype_alpha($params['pass'][1]) && array_key_exists($params['pass'][1], Configure::read('translations')) && $route = $this->route($connection, $params['pass'][2], $params['pass'][1], $params['pass'][0])){ // f.e. www.test.at/structure/en/bla-blub.html
                    $params['structure'] = $params['pass'][0];
                    $params['language'] = $params['pass'][1];
                    $params['route'] = $route;
                    $params['match'] = 'structure-language-route';
                }else if(strlen($params['pass'][0]) == 2 && ctype_alpha($params['pass'][0]) && array_key_exists($params['pass'][0], Configure::read('translations')) && $route = $this->route($connection, $params['pass'][1], $params['pass'][0], '')){ // f.e. www.test.at/en/bla-blub.html
                    $params['language'] = $params['pass'][0];
                    $params['route'] = $route;
        ...
        // params
        $params['error'] = false;
        $params['url'] = array_merge(['url' => $url], array_filter(explode('/', str_replace($this->ext, '', join("/", $params['pass'])))));
        $params['pass'] = !array_key_exists('keep', $params) ? [] : $params['pass'];
        
        // specials
        if(array_key_exists('match', $params) && is_string($params['match']) && in_array($params['match'], ['captcha','redirect','image','sitemap'])){
        ...
        if($params['error'] !== true){
            ...
            if($params['match'] == 'fallback'){
                ...
                $params['node'] = $connection->execute("SELECT `id`, `foreign_id`, `route`, `jump`, `popup`, `show_from`, `show_to`, `active`, `robots_follow`, `robots_index` FROM nodes WHERE active = 1 AND (show_from = '' OR show_from <= CURDATE()) AND (show_to = '' OR show_to > CURDATE()) AND structure_id = :structure AND parent_id = :parent ORDER BY position", ['structure' => $params['structure']['id'], 'parent' => ''])->fetch('assoc');
            }
            ...
            if(!array_key_exists('node', $params) || !is_array($params['node']) || count($params['node']) == 0){
                ...
                $params['element'] = $connection->execute("SELECT `id`, `code`, `category_id`, `show_from`, `show_to`, `active`, `valid_times`, `valid_fallback` FROM elements WHERE id = :id AND active = 1 AND (show_from = '' OR show_from <= CURDATE()) AND (show_to = '' OR show_to > CURDATE())", ['id' => $params['node']['foreign_id']])->fetch('assoc');
        ...
        if(array_key_exists('node', $url) && array_key_exists('language', $url)){
            ...
            if(strpos($url['node'], ":") !== false){
                ...
                if($type == 'node' && strlen($id) == 36){
                    ...
                    // get infos
                    $node = $connection->execute("SELECT `n`.`route`, `n`.`parent_id`, `t`.`content` FROM `nodes` as `n` LEFT JOIN `i18n` as `t` ON (`n`.`foreign_id` = `t`.`foreign_key`) WHERE `n`.`id` = :id AND `t`.`locale` = :locale AND `t`.`field` = :field LIMIT 1", ['id' => $id, 'locale' => $url['language'], 'field' => 'title'])->fetch('assoc');

                    if(is_array($node)){
                        
                        // parent
                        $parent = false;
                        if(strlen($node['parent_id']) == 36){
                            $parent = $connection->execute("SELECT `n`.`route`, `n`.`parent_id`, `t`.`content` FROM `nodes` as `n` LEFT JOIN `i18n` as `t` ON (`n`.`foreign_id` = `t`.`foreign_key`) WHERE `n`.`id` = :id AND `t`.`locale` = :locale AND `t`.`field` = :field LIMIT 1", ['id' => $node['parent_id'], 'locale' => $url['language'], 'field' => 'title'])->fetch('assoc');
                        }
                        
                        
                        // extend
                        $extend = array_key_exists('extend', $url) ? array_filter($url['extend']) : [];
                        $extend = count($extend) > 0 ? '/' . join("/", $extend) : '';

                        // params
                        $params = array_key_exists('?', $url) ? array_filter($url['?']) : [];
                        $params = count($params) > 0 ? '?' .  http_build_query($params) : '';                        
                        
                        // name
                        $name = [];
                        if(is_array($parent)){
                            $name[] = strtolower(Text::slug(str_replace($this->s,$this->r,html_entity_decode(strip_tags($parent['content'])))));
                        }
                        $name[] = strtolower(Text::slug(str_replace($this->s,$this->r,html_entity_decode(strip_tags($node['content'])))));
                        
                        // build url
                        $res = '/' . $url['language'] . '/' . join("-", $name) . $extend . $this->ext . $params;
    ...
    private function route($connection, $route, $language, $structure){
        
        // init
        $structure = $this->structure($connection, ['structure' => $structure]);
        $route = str_replace($this->ext, '', $route);
        
        // fetch routes
        if(!array_key_exists($structure['structure']['id'], $this->routes)){
            $nodes = $_nodes = [];
            $__nodes = $connection->execute("SELECT `n`.`id`, `n`.`route`, `n`.`parent_id`, `t`.`content` FROM `nodes` as `n` LEFT JOIN `i18n` as `t` ON (`n`.`foreign_id` = `t`.`foreign_key`) WHERE `n`.`structure_id` = :structure AND `t`.`locale` = :locale AND `t`.`field` = :field", ['structure' => $structure['structure']['id'], 'locale' => $language, 'field' => 'title'])->fetchAll('assoc');
            foreach($__nodes as $node){
                $node['content'] = strtolower(Text::slug(str_replace($this->s,$this->r,html_entity_decode(strip_tags($node['content'])))));
                $_nodes[$node['id']] = $node;
            }
            foreach($_nodes as $node){
                $parts = [];
                if(strlen($node['parent_id']) == 36 && array_key_exists($node['parent_id'], $_nodes)){
                    $parts[] = $_nodes[$node['parent_id']]['content'];
                }
                $parts[] = $node['content'];
                $nodes[join("-", $parts)] = $node['route'];
            }
            $this->routes[$structure['structure']['id']] = $nodes;
        }
        
        // search
        if(array_key_exists($structure['structure']['id'], $this->routes) && is_array($this->routes[$structure['structure']['id']]) && array_key_exists($route, $this->routes[$structure['structure']['id']])){
            return $this->routes[$structure['structure']['id']][$route];
        }
